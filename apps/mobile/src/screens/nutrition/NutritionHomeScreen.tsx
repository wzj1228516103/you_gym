...
export function NutritionHomeScreen({ navigation }: Props) {
  ...
  return <AppScreen>
    ...
    <SectionHeader title="营养提示" /><Card><Text style={styles.tip}>{todayRecords.length ? '今日记录已同步。营养目标尚未设置，当前仅展示已记录摄入量。' : '记录每餐食物后，这里会展示当天的实际摄入量。'}</Text></Card>
  </AppScreen>;
}
