...
export function PlanLibraryScreen({ navigation }: Props) {
  ...
  useFocusEffect(useCallback(() => {
    ...
    void fetchPlans(category).then((result) => { if (active) { setPlans(result.items); setError(null); } }).catch((cause) => { if (active) setError(cause instanceof Error ? cause.message : '计划加载失败'); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [category]));
  return (
    <AppScreen>
      ...
      <View style={styles.countRow}><Text style={styles.count}>精选计划</Text><Text style={styles.countMeta}>{loading ? '加载中…' : `${plans.length} 个`}</Text></View>
      ...
      {plans.map((plan) => (
        <Card key={plan.id} onPress={() => navigation.navigate('PlanDetail', { planId: plan.id })} accessibilityLabel={`查看${plan.title}`} style={styles.planCard}>
          ...
          <View style={styles.planCopy}>
            ...
            <Text style={styles.meta}>{plan.durationLabel} · {plan.exerciseCount} 个动作</Text>
