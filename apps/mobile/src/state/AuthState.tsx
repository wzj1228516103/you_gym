...
import { migrateGuestFavoriteExercises, migrateGuestFavoritePlans } from '../services/favorites';
...
export function AuthStateProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => { void (async () => { const stored = await loadStoredSession(); if (stored) { try { const current = await fetchMe(stored.accessToken); setToken(stored.accessToken); setUser(current); setAnalyticsUser(current.id); try { await migrateGuestFavoriteExercises(stored.accessToken, current.id); } catch { /* Keep local guest data for a later retry if sync is unavailable. */ } try { await migrateGuestFavoritePlans(stored.accessToken, current.id); } catch { /* Keep local guest data for a later retry if sync is unavailable. */ } } catch { await clearSession(); } } setLoading(false); })(); }, []);
  const value = useMemo<AuthStateValue>(() => ({ loading, token, user, guest,
    authenticate: async (accessToken, current) => { setToken(accessToken); setUser(current); setGuest(false); setAnalyticsUser(current.id); await saveSession({ accessToken, user: current }); try { await migrateGuestFavoriteExercises(accessToken, current.id); } catch { /* A failed sync must not block login. */ } try { await migrateGuestFavoritePlans(accessToken, current.id); } catch { /* A failed sync must not block login. */ } },
